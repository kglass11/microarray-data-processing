...
workdir <- "I:/Drakeley Group/PROTEIN MICROARRAYS/Experiments/230418 Human Pk case-control Qdot/IgA"
...
for(i in 1:length(slide_ids)) { 
  ...
  slides_list[[i]] <- read.table(slide_ids[i],skip=32,sep="\t",header=T)
...
# Use for IgA and IgM
if(iso == "IgA" | iso == "IgM"){
high_targets <- c(targets_ref, grep(paste(iso, "(200ug/Al)"), row.names(annotation_targets.df)))
}
...
png(filename = paste0(study,"_background_mfi_samples.tif"), width = 3.5, height = 9, units = "in", res = 600)
...
rmsamp_all <- unique(c(targets_blank, targets_buffer, targets_ref, high_targets_disinclude))
...
#Plot GST values
png(filename = paste0(study, "_GST.tif"), width = 3, height = 3, units = "in", res = 1200)
par(mfrow=c(1,1), oma=c(3,1,1,1),mar=c(4.1,4.1,3.1,2.1))
plot(GST_val, pch='*', col = "blue", ylim=c(0,max(GST_val)*1.25), main = "GST",
     ylab="Normalized log2(MFI)", xlab="Sample (Array)", cex.main=0.5, cex.lab=0.5, cex.axis=0.5)
...
save.image(file="Pk_HCC_analysis_IgA_V02.RData")
...
load("Pk_HCC_analysis_IgA_V02.RData")

#adding control samples ran with macaque samples; these are not integrated into sample_meta_f.df yet!!!
#Also, will need to get seropositivity information for these sample from the relevant study
macaque_norm_sub5 <- read.csv(file="Macaque_IgA_GST_subtracted_Final.csv")
norm_sub5.2.df <- cbind(norm_sub5.df, macaque_norm_sub5[,162:170])
...
#calculate number of seropositive people for all antigens
SPpeople <- as.matrix(sort(rowSums(seropos.matrix), decreasing = TRUE))
SPpeople <- as.data.frame(SPpeople)
SPpeople <- tibble::rownames_to_column(SPpeople)
colnames(SPpeople) <- c("Name", "Seropositive")




##################################################
####### Analysis for Seropositive data ONLY ######

#need to make a data frame all antigens, but only seropositive data, NA for everything else
...
#seropositives only, filtering out Lou antigens
...
tacos.df <- tacos.df[,sapply(tacos.df, is.numeric)]

#transpose Lou antigens
tacosT.df <- as.data.frame(t(tacos.df))
tacosT.df <- tibble::rownames_to_column(tacosT.df, var = "sample_id_unique")

#make the day column a character and not numeric
...
#merging with the sample metadata
burrito.df <- merge(sample_meta_f.df,tacosT.df, sort = FALSE)

#melt the data
meltburrito.df <- melt(burrito.df, na.rm = TRUE) 
meltantigens.df <- filter(meltburrito.df, !(variable == "Age" | variable == "parasitecount"))

#explicitly set factor levels to the correct order for day
meltantigens.df$day <- as.factor(meltantigens.df$day)
meltantigens.df$day <- factor(meltantigens.df$day, levels = c("0", "7", "28"))

#jitter plot of seropositive Lou antigen data, all antigens by time point
png(filename = paste0(study, "_PkSP_by_day_jit.tif"), width = 8, height = 5, units = "in", res = 1200)
par(mfrow=c(1,1), oma=c(3,1,1,1),mar=c(4.1,4.1,3.1,2.1))

ggplot(meltantigens.df, aes(x=day, y=value, fill = variable, color = variable)) + geom_jitter(width = 0.2) + xlab("Day") + ylab("Normalized Log2(MFI)") + 
  theme(text = element_text(size=9)) + theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + ylim(0,6)+
  theme(panel.border = element_blank(), axis.line = element_line(), panel.grid = element_blank())

graphics.off()

#line plot of seropositive Lou antigen data, all antigens by time point
#THIS IS NOT WORKING YET!!!
#need to have the lines connecting by StudyID - maybe different plot type than geom_line
png(filename = paste0(study, "_PkSP_by_day_line.tif"), width = 8, height = 5, units = "in", res = 1200)
par(mfrow=c(1,1), oma=c(3,1,1,1),mar=c(4.1,4.1,3.1,2.1))

ggplot(meltantigens.df, aes(x=as.factor(day), y=as.numeric(value), color = variable)) + geom_point(shape=18, size = 2) +
  geom_line(aes(group = variable)) + theme_bw()+ labs(x = "Day",y = "Normalized Log2(MFI)", title = "Antigen reactivity by time point") +
...
##################################################################
####### Analysis for ALL DATA, regardless of seropositivity ######
...
#merge target2.df with SPpeople to get metadata with SPpeople info
target3.df <- merge(target2.df, SPpeople, sort = FALSE)
...
#just samples and antigens - no metadata
meltLou <- melt(as.matrix(subLouT.df), na.rm = TRUE)
colnames(meltLou) <- c("Sample", "Antigen", "Normalized")

#metadata attached
Louness.df <- merge(sample_meta_f.df,subLouT2.df, sort = FALSE)
meltLouness <- melt(Louness.df, na.rm=TRUE)

#explicitly set factor levels to the correct order for day
meltLouness$day <- as.factor(meltLouness$day)
meltLouness$day <- factor(meltLouness$day, levels = c("0", "7", "28"))

meltedcheese <- filter(meltLouness, !(variable == "Age" | variable == "parasitecount"))

#explicity set factor levels for antigen to highest to lowest median value
#this isn't working yet
# meltedcheese$variable <- as.factor(meltedcheese$variable)
# meltedcheese$variable <- factor(meltedcheese$variable, levels = unique(reorder(meltLouness$variable, -meltLouness$value, FUN=median)))


# old code for melting - specifying id variables manually
# ids <- colnames(sample_meta_f.df) 
# meltLouness <- melt(Louness.df, id.vars = as.vector(colnames(sample_meta_f.df)), measure.vars = as.vector(colnames(subLouT.df)), 
#              variable.name = "Antigen", value.name = "Normalized", na.rm = TRUE)

#At this point, Remove control samples for further analysis
norm_sub6.df <- norm_sub5.df[,colnames(norm_sub5.df) %in% samples_test]

#Box Plots of data for reactive Lou antigens, sorted by highest median to lowest, by day

png(filename = paste0(study, "_Lou_boxplot_day.tif"), width = 8, height = 5, units = "in", res = 1200)
par(mfrow=c(1,1), oma=c(3,1,1,1),mar=c(4.1,4.1,3.1,2.1))

ggplot(meltedcheese, aes(x=reorder(variable, value, FUN=median), y=value, fill = day)) + geom_boxplot(outlier.size = 0.3) + xlab("Antigen") +
    ylab("Normalized Log2(MFI)") + theme_bw() + theme(text = element_text(size=9)) +
...
png(filename = paste0(study, "_Lou_boxplot_day_short.tif"), width = 8, height = 5, units = "in", res = 1200)
par(mfrow=c(1,1), oma=c(3,1,1,1),mar=c(4.1,4.1,3.1,2.1))

ggplot(meltedcheese, aes(x=reorder(variable, value, FUN=median), y=value, fill = day)) + geom_boxplot(outlier.size = 0.3) + xlab("Antigen") +
  ylab("Normalized Log2(MFI)") + theme_bw() + theme(text = element_text(size=9)) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, size = 6)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + ylim(c(0,3))+
  theme(panel.border = element_blank(), axis.line = element_line(), panel.grid = element_blank())
...
#line plot of ALL Lou antigen data, all antigens by time point
#THIS IS NOT WORKING YET!!!
#need to have the lines connecting by StudyID - maybe different plot type than geom_line
png(filename = paste0(study, "_PkALL_by_day_line.tif"), width = 8, height = 5, units = "in", res = 1200)
par(mfrow=c(1,1), oma=c(3,1,1,1),mar=c(4.1,4.1,3.1,2.1))

ggplot(meltedcheese, aes(x=as.factor(day), y=as.numeric(value), color = variable)) + geom_point(shape=18, size = 2) +
  geom_line(aes(group = variable)) + theme_bw()+
  labs(x = "Day",y = "Normalized Log2(MFI)", title = "Antigen reactivity by time point") +
  theme(text = element_text(size=9)) + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+
  theme(panel.border = element_blank(), axis.line = element_line(), panel.grid = element_blank())
...
ggplot(meltedcheese,aes(day,sample_id,fill=value))+
  geom_tile(color= "purple",size=0.1) +
...
ggplot(meltedcheese, aes(variable, sample_id, z= value)) + geom_tile(aes(fill = value)) + 
  theme_bw() + 
...
ggplot(meltedcheese, aes(x=reorder(variable, value, FUN=median), y=sample_id, z= value)) +
...
#scatter plot of all antigens, color for gender

ggplot(meltLouness, aes(x=reorder(Antigen, -Normalized, FUN=median), y=Normalized, color = gend)) + geom_point()

png(filename = paste0(study, "_Lou_scatter_all_gender.tif"), width = 5, height = 8, units = "in", res = 1200)
par(mfrow=c(1,1), oma=c(3,1,1,1),mar=c(4.1,4.1,3.1,2.1))

ggplot(meltLouness, aes(x=reorder(Antigen, Normalized, FUN=median), y=Normalized, color = gend)) + geom_point() +
  coord_flip() + xlab("Antigen") + ylab("Normalized Log2(MFI)") + theme(text = element_text(size=9))

graphics.off()

#Boxplot of all antigens, fill by gender

ggplot(meltLouness, aes(x=reorder(Antigen, -Normalized, FUN=median), y=Normalized, fill = gend)) + geom_boxplot()

png(filename = paste0(study, "_Lou_boxplot_all_gender.tif"), width = 8, height = 5, units = "in", res = 1200)
par(mfrow=c(1,1), oma=c(3,1,1,1),mar=c(4.1,4.1,3.1,2.1))

ggplot(meltLouness, aes(x=reorder(Antigen, -Normalized, FUN=median), y=Normalized, fill = gend)) + 
  geom_boxplot() + theme_bw() + xlab("Antigen") + ylab("Normalized Log2(MFI)") + theme(text = element_text(size=9)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))

graphics.off()

#Plot all antigens by age
ggplot(meltLouness, aes(x=age, y=Normalized, color = Antigen)) + geom_point()

png(filename = paste0(study, "_Lou_ALLantigens_Age.tif"), width = 5, height = 8, units = "in", res = 1200)
par(mfrow=c(1,1), oma=c(3,1,1,1),mar=c(4.1,4.1,3.1,2.1))

ggplot(meltLouness, aes(x=reorder(Antigen, Normalized, FUN=median), y=Normalized)) + geom_violin() + coord_flip() + 
  xlab("Antigen") + ylab("Normalized Log2(MFI)") + theme(text = element_text(size=9)) +
  

graphics.off()

#Plot the number of seropositive people by antigen, highest to lowest for Lou

#Repeat this by day!!!! 

LouSP.df <- filter(target3.df, Source == "Lou")

LouSP.df$Name <- as.factor(LouSP.df$Name)
#explicitly set factor levels to the correct order
LouSP.df$Name <- factor(LouSP.df$Name, levels = LouSP.df$Name[order(-LouSP.df$Seropositive)])

png(filename = paste0(study, "_Lou_All_SeropositivePeople.tif"), width = 8, height = 4.5, units = "in", res = 1200)
par(mfrow=c(1,1), oma=c(3,1,1,1),mar=c(4.1,4.1,3.1,2.1))

ggplot(LouSP.df, aes(x = Name, y = Seropositive)) + theme_bw() + geom_bar(stat="identity") + ylab("Number of Seropositive Individuals") + 
  xlab("Antigen") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, size = 6))
