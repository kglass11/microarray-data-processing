...
#Adjust the corrected dataframe to get rid of zeros and normalise the values close to zero, using the limma package
  #KG added this based on Will's parameters from line 326.
cor.matrix.limma <- backgroundCorrect.matrix(fore.matrix, back.matrix, method = "normexp", offset = 0, normexp.method = "mle")
log.cor.matrix <- log2(cor.matrix.limma)

#Export this for reference
write.csv(cor.matrix, file="cor.matrix.csv")
write.csv(cor.matrix.limma, file = "cor.matrix.limma")
...
#SD median background MFI for all data points
...
# KG - I have changed the buffer analyses to use the limmanormexp background correction (cor.matrix.limma) and not the background subtraction method. 

#Mean median corrected MFI for all data points
cor_buffer_mean <- mean(cor.matrix.limma[targets_buffer,])
#SD median corrected MFI for all data points
cor_buffer_sd <- sd(cor.matrix.limma[targets_buffer,])
#Cut-off for deviation from mean
...
cor_buffer_sample_mean <- colMeans(cor.matrix.limma[targets_buffer,])
#Vectors to identify normal and deviant samples
...
#Coefficient of variation for all background datapoints, and all exlcuding deviant samples
cor_buffer_cov_all <- sd(cor.matrix.limma[targets_buffer,])/mean(cor.matrix.limma[targets_buffer,])
cor_buffer_cov_all_normal <- sd(cor.matrix.limma[targets_buffer, cor_normal])/mean(cor.matrix.limma[targets_buffer, cor_normal])

#Plots by slide/pad/sample
png(filename = paste("cor_mfi_all.tif"), width = 5.5, height = 10, units = "in", res = 600)
par(mfrow=c(3,1), mar = c(2, 3, 2.25, 0.5), oma = c(11.5, 0, 1, 0), bty = "o", 
    mgp = c(2, 0.5, 0), cex.main = 1.5, cex.axis = 0.6, cex.lab = 0.9, xpd=NA, las=1)
boxplot(t(cor.matrix.limma[targets_buffer,]) ~ samples.df$slide_no, outcex=0.5,
...
boxplot(t(cor.matrix.limma[targets_buffer,]) ~ samples.df$block_rep_1, outcex=0.5,
        ylab="Corrected MFI", xlab="Subarrays (1/2, 3/4, etc.)", add=FALSE, las=1)
abline(h = cor_cutoff, col = "red", lty = 2, lwd = 0.7, xpd=FALSE)
boxplot(t(cor.matrix.limma[targets_buffer,]) ~ samples.df$sample_id_unique, outcex=0.5,
...
cor_target_mean <- rowMeans(cor.matrix.limma)
...
boxplot(t(cor.matrix.limma[targets_buffer,]),
        cex=0.5,
        ylab="Raw MFI")
abline(h = cor_cutoff, col = "red", lty = 2, lwd = 0.7, xpd=FALSE)
graphics.off()
...
for (i in 1:index_slide){
  boxplot(t(cor.matrix.limma[targets_buffer,samples.df$slide_no==i]),
          ylab="Raw MFI",
          ylim=c(0,2000),
          add=FALSE, 
          cex=0.5,
...
for(i in 1:ncol(cor.matrix.limma))
{
  temp <- sd(cor.matrix.limma[targets_buffer,i])/mean(cor.matrix.limma[targets_buffer,i])
...
#KG - We don't need to do this anymore, it is already taken care of within the limma background correction method.
cor2.matrix <- cor.matrix.limma
length(cor2.matrix)
sum(cor2.matrix<=0)
sum(cor2.matrix>0)
...
#Checking standards
plot(cor.matrix.limma[targets_std,1])
axis(side=1, at=c(1:12), labels=rownames(cor.matrix.limma[targets_std, ]))
...
#2. Corrected for zeros
  ...
  #### this section doesn't work because there is no object called median.matrix. might work with using fore.matrix
  ## but I don't think we need to include this.
...
cat("Number of negative values in UNCORRECTED matrix", sum(median.matrix < 1), "out of", sum(cor2.median.matrix>0), "(", sum(median.matrix < 1)/sum(cor2.median.matrix>0)*100, "%)") 
cat("Number of negative values in UNCORRECTED matrix", sum(cor.median.matrix < 1), "out of", sum(cor2.median.matrix>0), "(", sum(cor.median.matrix < 1)/sum(cor2.median.matrix>0)*100, "%)") 
cat("Number of negative values in CORRECTED matrix", sum(cor.median.matrix < 1))
